-- ===============================================================
-- Consolidado: bugfixes aplicados vía Management API (jul 2026)
-- Estos cambios YA están en producción.
-- Para sincronizar: supabase db push
-- ===============================================================

-- 1. check_username_available → SECURITY INVOKER + muzicmania.profiles
...
-- 2. get_email_by_username → search_path + muzicmania.profiles
...
-- 3. handle_account_deletion → search_path + muzicmania.profiles
...
-- 4. is_account_recoverable(TEXT) → muzicmania.profiles
...
-- 5. handle_new_user → search_path + evitar public.profiles (bug crítico)
...
BEGIN
  ...
  INSERT INTO profiles (
    ...
    new.email, new.raw_user_meta_data->>'country',
...
-- 6. muzicmania.submit_game_score → evitar public.profiles (bug crítico)
...
BEGIN
  ...
  INSERT INTO scores (user_id, track_id, score, combo, accuracy, grade,
    max_combo, perfect, great, good, miss)
  VALUES (v_user_id, p_track_id, p_score, p_combo, p_accuracy, p_grade,

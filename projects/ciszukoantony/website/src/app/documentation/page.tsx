...
export default function DocumentationPortal() {
  ...
  const fetchContent = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const identifier = `${selectedDoc}_V${meta.version}_${meta.lastUpdate.replace(/-/g, '_')}_CISZUKOANTONY`;
        const header = format === 'txt' ? `DOCUMENTATION - CISZUKO ANTONY PORTAL\nDOC_ID: ${selectedDoc}\nIDENTIFIER: ${identifier}\n------------------------------------------------\n\n` : '';
        const cleanText = text.replace(/^CISZUKO ANTONY - DOCUMENTACIÓN OFICIAL[\s\S]*?---\n\n/, '')
                             .replace(/^# CISZUKO ANTONY - DOCUMENTACIÓN OFICIAL[\s\S]*?---\n\n/, '');
  ...
  const handlePackageDownload = (pkgExt: string) => {
    const identifier = `${selectedDoc}_V${meta.version}_${meta.lastUpdate.replace(/-/g, '_')}_CISZUKOANTONY_PKG`;
    const randomSuffix = generateRandomSuffix();
      ...
      >
        <div className="flex flex-col items-center gap-1 text-center">
          <div className="flex items-center justify-center gap-6 group">
            ...
            <h1 className="text-4xl md:text-8xl font-header font-black uppercase tracking-tighter leading-none transition-all group-hover:tracking-normal bg-gradient-to-r from-neon-blue via-white to-neon-pink bg-clip-text text-transparent" style={{ WebkitTextStroke: '1px black' }}>
              DOCUMENTACIÓN
            </h1>
          </div>
                ...
                {filteredDocs.map(id => (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-2xl flex items-center gap-4 transition-all group ${
                      selectedDoc === id ? 'bg-neon-blue/10 border border-neon-blue/30 text-neon-blue shadow-[0_0_20px_rgba(61,106,223,0.1)]' : 'hover:bg-white/5 text-gray-500 border border-transparent'
                    }`}
                  >
            ...
            <div className="p-8 bg-doc-dark border border-white/10 rounded-[2.9rem] space-y-6">
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="flex justify-between border-b border-white/5 pb-2">
                  ...
                  <span className="text-[9px] font-black text-white font-mono uppercase">CA_NODE_{selectedDoc.slice(0,3)}</span>
                </div>
              </div>
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-10">
              <div className="space-y-6 flex-1 min-w-0">
                ...
                <div className="flex items-center gap-6">
                  <div className="w-12 h-12 text-neon-blue flex-shrink-0" style={{ filter: 'drop-shadow(0 0 8px rgba(61,106,223,0.5))' }}>{(I as any)[meta.icon] || I.book}</div>

    ...
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-4xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center items-center gap-6 relative z-10">
              <button
                ...
                className={`flex items-center gap-3 px-6 py-3 rounded-2xl bg-white/5 border transition-all group/like ${
                  likes.isLiked(item.id) ? 'border-brand-accent text-brand-300' : 'border-white/10 hover:border-brand-accent'
                }`}
              >
                <div className="w-5 h-5 group-hover/like:scale-110 transition-all">{I.heart}</div>
                <span className="text-[12px] font-mono font-black text-white/50 group-hover/like:text-brand-300">
              ...
              {related.map((rel) => {
                ...
                return (
                  <Link
                    ...
                    className="group p-6 rounded-[2rem] bg-white/5 border border-white/5 hover:border-brand-light/40 hover:bg-white/10 transition-all flex flex-col gap-3"
                  >
                    ...
                    <span className="text-sm font-header font-black text-white uppercase italic tracking-tighter leading-tight group-hover:text-brand-300 transition-colors">
                      {rel.title}
                    </span>
        ...
        <motion.section
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.7, ease: 'easeOut' }}
          className="p-px rounded-[4rem] bg-gradient-to-r from-brand-light/30 via-brand/20 to-transparent mb-20"
        >

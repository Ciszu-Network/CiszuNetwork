...
export default function ChangelogPage() {
  ...
  const published = usePublishedChangelogs('ciszukoantony');
      ...
      <PageReveal className="relative mx-auto max-w-screen-xl">
          ...
          <div className="flex items-center justify-between relative z-10 gap-6">
            <div className="space-y-1">
              <h2 className="text-xl font-header font-black text-white uppercase italic tracking-tighter flex items-center gap-3">
                <div className="w-5 h-5 text-brand-300">{I.zap}</div>
          ...
          <div className="p-10 bg-black/60 border border-white/10 rounded-[3rem] space-y-10 relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-10 relative z-10">
              <div className="absolute top-1/2 left-8 right-8 h-1 bg-white/5 -translate-y-1/2 hidden md:block rounded-full overflow-hidden">
                <div className="h-full w-1/2 bg-gradient-to-r from-brand to-brand-light animate-pulse" />
              </div>
                ...
                <div key={step.label} className="relative flex flex-col items-center gap-4 w-full md:w-1/4 group/node">
                  <div
                    className={`relative w-20 h-20 rounded-[2rem] flex items-center justify-center border-2 transition-all duration-500 z-10 ${
                      step.status === 'done'
                        ? 'bg-brand/20 border-brand-light text-brand-light group-hover/node:scale-110'
                        : step.status === 'next'
                          ? 'bg-brand-light/20 border-brand-light text-white animate-pulse group-hover/node:scale-110'
                  ...
                  <div className="text-center space-y-1 bg-black/95 p-4 rounded-3xl border border-white/10 w-full z-10 group-hover/node:bg-white/10 transition-colors">
                    <div
                      className={`text-[11px] font-black uppercase tracking-tighter italic ${
                        step.status === 'done' ? 'text-brand-light' : step.status === 'next' ? 'text-brand-300' : 'text-white/40'
                      }`}
                    >
                      {step.label}
                    </div>
                    ...
                    <div
                      className={`text-[7px] font-black uppercase tracking-[0.2em] mt-2 pt-2 border-t border-white/10 ${
                        step.status === 'done' ? 'text-brand-light' : step.status === 'next' ? 'text-brand-300' : 'text-white/20'
                      }`}
                    >
        ...
        <Section className="mb-8">
          <div className="relative group max-w-2xl mx-auto">
            <div className="absolute inset-y-0 left-8 flex items-center pointer-events-none text-white/20 group-focus-within:text-brand-light transition-colors">
            ...
            <div className="flex items-center gap-4">
              <button
                ...
                className={`flex items-center gap-4 px-8 py-4 rounded-2xl border transition-all font-black text-[12px] tracking-[0.2em] group ${
                  showTagSelector ? 'bg-brand-light border-brand-light text-white shadow-lg' : 'bg-black/40 border-white/10 text-white/40'
                }`}
              >
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-[9px] font-black uppercase tracking-widest transition-all ${
                    isActive
                      ? 'bg-brand-light/20 border-brand-light/40 text-brand-light'
                ...
                >
                  ...
                  <span className="text-brand-300/60">{count}</span>
                </button>
              );
            })}
          </div>
        ...
        <div className="space-y-10 mb-16">
          {paginatedData.length === 0 ? (
            ...
            paginatedData.map((item) => (
              <div
                key={item.id}
                className="group relative p-px rounded-[3rem] bg-gradient-to-br from-white/10 to-transparent hover:from-brand-light/40 transition-all duration-700"
              >
                  ...
                  <div className="flex flex-col md:flex-row gap-10 items-start md:items-center">
                    ...
                    <div className="flex-1 space-y-6">
                      ...
                      <div className="space-y-4">
                        <div className="space-y-2">
                          <h2 className="text-3xl md:text-4xl font-header font-black text-white group-hover:text-brand-300 transition-colors uppercase italic tracking-tighter leading-none">
                            {item.title}
                          </h2>
                    ...
                    <div className="flex md:flex-col items-center gap-6 self-stretch justify-between md:justify-center border-t md:border-t-0 md:border-l border-white/5 pt-8 md:pt-0 md:pl-12">
                      <Link
                        ...
                        className="px-8 py-4 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black text-white uppercase tracking-widest hover:bg-brand-light hover:text-black hover:border-brand-light transition-all group/btn flex items-center gap-3"
                      >
                        DETALLES
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center gap-1 group/like p-2 transition-colors ${
                          likes.isLiked(item.id) ? 'text-brand-300' : 'hover:text-brand-300'
                        }`}
                      >
                        <div className="w-6 h-6 group-hover/like:scale-125 transition-transform">{I.heart}</div>
                        <span className="text-[10px] font-mono font-black text-white/30 group-hover/like:text-brand-300">
          ...
          {Array.from({ length: page.totalPages }, (_, i) => i + 1).map((n) => (
            <button
              ...
              className={`w-12 h-12 rounded-xl font-header font-black transition-all border ${
                n === page.page ? 'bg-brand-light border-brand-light text-white shadow-lg' : 'bg-white/5 border-white/5 text-white/20 hover:bg-white/10'
              }`}
            >
              {n}
            </button>
              ...
              return (
                <div
                  ...
                  className={`p-8 rounded-[2.5rem] relative overflow-hidden group transition-colors ${
                    isCurrent
                      ? 'bg-gradient-to-br from-brand/20 to-transparent border border-brand-light/30'
                      : 'bg-white/5 border border-white/5 hover:border-brand-light/30'
                  }`}
                >
                    ...
                    <div>
                      ...
                      <span
                        className={`text-[10px] font-black tracking-[0.3em] uppercase ${
                          isCurrent ? 'text-brand-light' : 'text-white/20 group-hover:text-brand-300'
                        } transition-colors`}
                      >
                        ...
                        <div key={i} className="flex items-start gap-3 group/task">
                          <div
                            className={`mt-0.5 w-5 h-5 rounded-md flex items-center justify-center shrink-0 border transition-all ${
                              task.done
                                ? 'bg-brand-light/20 border-brand-light text-brand-light'
        ...
        {/* --- PROTOCOLOS / DOCS CTA --- */}
        <Section className="p-px rounded-[4rem] bg-gradient-to-r from-brand-light/30 via-brand/20 to-transparent mb-20">

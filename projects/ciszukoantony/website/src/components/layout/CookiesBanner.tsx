...
export function CookiesBanner({ lang, dict }: { lang: string; dict: Record<string, any> }) {
  ...
  const handleAccept = () => {
    ...
    setHasAcceptedCookies(true);
    setShow(false);
    toast('Cookies accepted. Thank you for supporting Ciszuko Antony.', 'info');
  };
  ...
  const handleReject = () => {
    ...
    setShow(false);
    toast('Cookies rejected: optional services are now disabled.', 'info');
  };
  ...
  return (
    <AnimatePresence>
      {show && (
        ...
        >
          <div className="pointer-events-auto max-w-4xl mx-auto bg-black/90 backdrop-blur-xl border-t border-neon-cyan/30 md:border md:rounded-[2rem] p-6 shadow-[0_0_40px_rgba(90,130,232,0.15)] flex flex-col md:flex-row items-center gap-6 justify-between">

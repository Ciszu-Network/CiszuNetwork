          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-4 bg-white/5 border border-white/5 rounded-[3rem] backdrop-blur-xl">
            <div className="flex items-center gap-4">
              <button
                ...
                className={`flex items-center gap-4 px-8 py-4 rounded-2xl border transition-all font-black text-[12px] tracking-[0.2em] group ${showTagSelector ? 'bg-brand-light border-brand-light text-white shadow-lg' : 'bg-black/40 border-white/10 text-white/40'}`}
              >
          ...
          {/* TAG DISTRIBUTION STATS */}
           <div className="flex flex-wrap gap-4 px-6">
              {Object.entries(tagStats).map(([tag, count]) => {
                ...
                return (
                 <div key={tag} className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-[9px] font-black uppercase tracking-widest transition-all ${isActive ? 'bg-brand-light/20 border-brand-light/40 text-brand-light' : 'bg-white/5 border-white/5 text-white/30 hover:border-white/20'}`}>
                    ...
                    <span className="text-brand-accent/60">{count}</span>
                 </div>
                );
              })}
           </div>
                        ...
                        <div className="flex flex-col md:flex-row gap-10 items-start md:items-center">
                            ...
                            <div className="flex-1 space-y-6">
                               <div className="flex flex-wrap items-center gap-4">
                                  ...
                                  <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 text-[9px] font-black text-white/40 uppercase tracking-widest border border-white/5">
                                     <svg viewBox="0 0 24 24" className="w-3 h-3 text-brand-light"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
                                     {item.author}
                                  </div>
                               </div>
                               ...
                               <div className="space-y-4">
                                  <div className="space-y-2">
                                     <h2 className="text-4xl font-header font-black text-white group-hover:text-brand-accent transition-colors uppercase italic tracking-tighter leading-none">
                                        {item.title}
                                     </h2>
                           ...
                           <div className="flex md:flex-col items-center gap-6 self-stretch justify-between md:justify-center border-t md:border-t-0 md:border-l border-white/5 pt-8 md:pt-0 md:pl-12">
                              ...
                              <button
                                ...
                                className="flex flex-col items-center gap-1 group/like p-2 hover:text-brand-accent transition-colors"
                              >
                                 ...
                                 <span className="text-[10px] font-mono font-black text-white/30 group-hover/like:text-brand-accent">{item.likes || 0}</span>
                              </button>
                           </div>
                        </div>
                    </div>
        ...
        <div className="flex items-center justify-center gap-3 mb-20">
           ...
           {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => (
             <button
               ...
               className={`w-12 h-12 rounded-xl font-header font-black transition-all border ${page === currentPage ? 'bg-brand-light border-brand-light text-white shadow-lg' : 'bg-white/5 border-white/5 text-white/20 hover:bg-white/10'}`}
             >
                {page}
             </button>
           ))}

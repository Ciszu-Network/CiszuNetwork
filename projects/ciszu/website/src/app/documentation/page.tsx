...
export default function DocumentationPortal() {
  ...
  const fetchContent = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const header = format === 'txt' ? `DOCUMENTATION - CISZU NETWORK PORTAL\nDOC_ID: ${selectedDoc}\nIDENTIFIER: ${identifier}\n------------------------------------------------\n\n` : '';
        const cleanText = text.replace(/^CISZU NETWORK - DOCUMENTACIÓN OFICIAL[\s\S]*?---\n\n/, '')
  ...
  const handlePackageDownload = (pkgExt: string) => {
    const identifier = `${selectedDoc}_V${meta.version}_${meta.lastUpdate.replace(/-/g, '_')}_CISZUNETWORK_PKG`;
    const randomSuffix = generateRandomSuffix();
                ...
                {filteredDocs.map(id => (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-2xl flex items-center gap-4 transition-all group ${
                      selectedDoc === id ? 'bg-neon-blue/10 border border-neon-blue/30 text-neon-blue shadow-[0_0_20px_rgba(89,180,255,0.1)]' : 'hover:bg-white/5 text-gray-500 border border-transparent'
                    }`}
                  >
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-10">
              <div className="space-y-6 flex-1 min-w-0">
                ...
                <div className="flex items-center gap-6">
                  <div className="w-12 h-12 text-neon-blue flex-shrink-0" style={{ filter: 'drop-shadow(0 0 8px rgba(89,180,255,0.5))' }}>{(I as any)[meta.icon] || I.book}</div>

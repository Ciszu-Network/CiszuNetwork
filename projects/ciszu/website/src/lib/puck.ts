...
import { eq } from "@ciszunetwork/db";
...
export async function getPuckPage(path: string): Promise<PuckPage | null> {
  const rows = await db.select().from(ciszuSchema.puckPages).where(eq(ciszuSchema.puckPages.path, path)).limit(1);
...
export async function savePuckPage(path: string, data: unknown): Promise<void> {
  await db.insert(ciszuSchema.puckPages)
    .values({ path, data })
    .onConflictDoUpdate({ target: ciszuSchema.puckPages.path, set: { data, updatedAt: new Date() } });
}

# Ciszu Network — Workflow de la app

Version: 1.0.0
Actualizacion: 2026-08-26
Identificador: WORKFLOW_APP_ciszu_PROTOCOLS_V1.0.0_2026_08_26_ciszunetwork

> **Definicion**: Como actua Ciszu Network pagina a pagina y su ciclo de vida.
...
- **`/guidelines`**: renderiza contenido; (si es dinamica) consulta Supabase/API; envio de eventos a GA4/PostHog.
- **`/policies`**: renderiza contenido; (si es dinamica) consulta Supabase/API; envio de eventos a GA4/PostHog.
- **`/support`**: renderiza contenido; (si es dinamica) consulta Supabase/API; envio de eventos a GA4/PostHog.
- **`/team`**: renderiza contenido; (si es dinamica) consulta Supabase/API; envio de eventos a GA4/PostHog.

## 4. Disparadores de GitHub Actions (workflows)

- CI: lint + test + typecheck + semgrep + audit + gitleaks (push/PR).
- Deploy: `deploy-ciszu-website.yml` (push a main, deploy a Vercel desde la raiz).

...
| **Chromatic** | **Añadido (F4, dev-only)** en `@ciszu/ui` (CLI 18.2) | Visual + a11y testing alojado de las stories. Builds 1–5 publicados (14 ago 2026; 5 stories/2 componentes; builds 4–5 vía publicación local por billing suspendido). Addon `@storybook/addon-a11y` + tags `a11y`. Token `CHROMATIC_PROJECT_TOKEN` en vault; script `chromatic`; CI en `.github/workflows/chromatic.yml` (action v18, TurboSnap, autoAccept en main). |
| **Figma** | **Añadido (F4, dev-only)** en `@ciszu/ui` (addon `@storybook/addon-designs` 11.1.4) | Incrusta diseños de Figma en la pestaña Design de las stories (`parameters.design = { type: 'figma', url }`). Declarado en `main.ts` → `addons`; ejemplos en `Icon.stories.tsx`/`SmartImage.stories.tsx`. URLs `https://www.figma.com/file/<FILE_ID>?node-id=<FRAME>`. Enlace bidireccional real vía plugin Figma "Storybook Connect" (requiere Storybook publicado en Chromatic). Detalle `PACKAGES_SYSTEM` §4. |

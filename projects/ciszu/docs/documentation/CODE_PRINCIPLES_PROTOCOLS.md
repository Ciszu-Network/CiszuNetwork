...
> **Skills del agente opcionales** (catálogo: `MODELS_SKILLS_SYSTEM.md`): el ingeniero puede
> cargar `brainstorming`/`writing-plans` (diseño previo), `test-driven-development`
> (tests primero), `systematic-debugging` (causa raíz), `verification-before-completion`
> (evidencia antes de "listo") y `requesting-code-review`/`receiving-code-review`
> (verificación y feedback). No sustituyen las reglas anteriores; se invocan bajo demanda.
...
> **Refuerzo opcional con skills del agente** (`MODELS_SKILLS_SYSTEM.md`): los pasos 1-2 pueden
> apoyarse en `brainstorming`/`writing-plans` (diseño), el 4 en `test-driven-development`
> (escribir el test primero), el 5 en `verification-before-completion` (evidencia real) y, si
> el paso 4 falla, `systematic-debugging` (causa raíz). Las skills no sustituyen los pasos.

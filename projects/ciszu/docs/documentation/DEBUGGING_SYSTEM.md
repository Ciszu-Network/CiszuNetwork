...
2. Encender la web afectada con el modo CLI: `ps ... -Action start -Web <key>`.
3. Tras editar, esperar el compilado (5-10 s), consultar `devstatus` para confirmar 200.
4. Leer `devlog <key>` si hay errores de compilación.
5. Correr `pnpm --filter <web> lint` (y `typecheck` si aplica).
6. Dejar la web encendida solo si Ciszuko va a probarla; si no, detenerla y liberar puertos.

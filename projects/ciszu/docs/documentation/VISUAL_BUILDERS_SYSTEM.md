...
Identificador: VISUAL_BUILDERS_SYSTEM_V1.2.0_2026_08_24_ciszunetwork
...
### 6.8 Fase 4 — Plasmic + Onlook como herramientas LOCALES (24 ago 2026)

**Decisión**: se implementan **Plasmic** y **Onlook** como herramientas complementarias de diseño
visual que **trabajan en paralelo y NO se sobreponen** con Puck. Cada una edita en su propio
dominio:

| Herramienta | Dominio | Dónde vive | Uso |
| --- | --- | --- | --- |
| **Puck** | Page-builder embebido (bloques propios) | `/edit/*` en las 4 webs (BD) | Construir páginas con bloques de marca |
| **Plasmic** | Visual builder externo (Studio + loader) | Studio en navegador → `/plasmic/*` local | Diseñar páginas/componentes y servirlos en dev |
| **Onlook** | ⏸️ **Pospuesto** (demo / self-host pesado) | `clones/onlook` (gitignored) | Retomar cuando madure |

**Requisito común**: **SIEMPRE local, NUNCA en producción**. Las 3 usan el dev server; la
superficie de edición no existe (o responde 404) en Vercel.

#### 6.8.1 Plasmic (integrado en las 4 webs)

- **Paquete**: `@plasmicapp/loader-nextjs@2.0.24` en cada `projects/<web>/website` (dependencia
  regular; el runtime en producción no carga nada si no hay envs).
- **Archivos por web** (idénticos en las 4):
  - `src/lib/plasmic-init.ts` — `initPlasmicLoader` con `PLASMIC_PROJECT_ID` +
    `PLASMIC_PUBLIC_TOKEN`; `preview: true` solo en dev. **Si falta alguna env → `PLASMIC = null`**
    (caso producción).
  - `src/lib/plasmic-init-client.tsx` — `ClientPlasmicRootProvider` (no-op si `PLASMIC` es null).
  - `src/app/plasmic-host/page.tsx` — hook del Studio (`/plasmic-host`, render `PlasmicCanvasHost`).
  - `src/app/plasmic/[[...path]]/page.tsx` — catch-all que renderiza páginas diseñadas bajo
    `/plasmic/*` (`notFound()` si `PLASMIC` es null o la ruta no existe).
- **Env local** (`projects/<web>/website/.env.local`, gitignored):
  `PLASMIC_PUBLIC_TOKEN` (valor real del vault/Bitwarden) + `PLASMIC_PROJECT_ID` (ID del proyecto
  en `studio.plasmic.app/projects/<ID>` — se rellena al crear/vincular el proyecto).
- **NUNCA** en Vercel: sin envs, `/plasmic-host` y `/plasmic/*` responden 404.
- **Verificación**: tsc + lint OK en las 4 webs.

#### 6.8.2 Onlook (⏸️ POSPUESTO — estado real 24 ago 2026)

- **Estado actual**: Onlook está en **modo demo / en reconstrucción activa** (self-host con
  `clones/onlook` clonado e instalado, 24 ago 2026). **NO se considera factible implementarlo
  hoy**: su self-host exige infraestructura pesada y keys externas, y la herramienta está
  cambiando. **Dejado para después** — no bloquear el flujo de editores.
- **Qué quedó** (local, gitignored `clones/`):
  - Clonado en `clones/onlook` (repo `onlook-dev/onlook`, Apache 2.0), `bun install` OK.
  - `.env` base copiados en `apps/web/client`, `apps/backend`, `packages/db`.
  - Requisitos pendientes (intervención manual): Docker + `bun backend:start` (Supabase local),
    **CodeSandbox API key**, **OpenRouter key**, **MorphLLM o Relace** (fast-apply);
    después `bun run setup:env` → `bun db:push` → `bun db:seed` → `bun dev`.
- **Cuándo retomarlo**: cuando el self-host de Onlook madure (o si se accede a la versión hosted).
  Mientras, **Plasmic** (§6.8.1) y **Puck** cubren el editor visual local.
...
| 2026-08-24 | Fase 4 (§6.8): **Plasmic** integrado en las 4 webs como visual builder LOCAL (`@plasmicapp/loader-nextjs@2.0.24`, `/plasmic-host` + `/plasmic/[[...path]]`, envs `PLASMIC_PROJECT_ID`/`PLASMIC_PUBLIC_TOKEN` solo en `.env.local`; sin envs → 404). **Onlook** clonado e instalado en `clones/onlook` pero **POSPUESTO** (demo / self-host pesado: Docker + Supabase local + CodeSandbox + OpenRouter + Morph/Relace; intervención manual pendiente). tsc + lint + build OK en las 4 webs. |
...
_Última revisión: 24 ago 2026._ Relacionado: `FRONTEND_SYSTEM.md`, `UI_COMPONENTS_SYSTEM.md`,

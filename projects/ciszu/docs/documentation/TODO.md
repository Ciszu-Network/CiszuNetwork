...
- [ ] La seccion de informacion en el header debe estar al final de la fila de paginas mostrables en el navbar. CiszuNetwork no tiene seccion de information.
- [ ] Replicar sistema de documentacion de muzicmania correctamente a las demas paginas, debemos regenerar la docuementacion de cada pagina en cada formato, subirlo al cdn, crear los paquetes comprimibles. Y finalmente crear la estructura visual de muzicmania documentation a las demas paginas. Sin contar la carpeta documentation. Opciones de descarga, versiones, preview, descarga individual, info, bibloteca de documentacion etc.
- [ ] Replicar nuevo navbar de ciszubot a las demas paginas pero corregido. Actualmente el popup flotante del nombre de la pagina al hacer hover se ve mal exepto para ciszubot. El fondo de la pildora es transparente y el titulo no se ve correctamente, debe tener fondo determinado. Otro problema claro es la cantidad de paginas en el header de ciszubot. En zoom 100% la seccion de information se esconde, debemos acomodar y corregir esto.
- [ ] En los soportes de cada website exepto muzicmania. Los correos deberian estar en miniscula, boton de copiar y mailto. Ademas veo que estas usando diferentes iconso por cada website de soporte, el icono correcto es una boya salvavidas circular.
...
- [ ] Agregar pagina de donacion a los que no tienen. Se deben utilizar iconos REALES no generados por IA, se deben descargar oficialmente el svg y agregarlo a los icons del CDN.

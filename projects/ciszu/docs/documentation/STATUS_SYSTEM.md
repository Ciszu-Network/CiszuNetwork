...
Identificador: STATUS_SYSTEM_V2.2.1_2026_08_24_ciszunetwork
...
| Compilación local de webs | ✅ Las 4 webs compilan y cargan assets correctamente en dev (puertos 3000–3003, CDN local 8788); el reporte previo de "assets no cargan" en ciszukoantony correspondía a servers viejos con puertos cruzados — 19 ago 2026 |
| Navbar ciszubot (orden botones) | ✅ Botón AUTH movido **después** del botón hamburguesa — 19 ago 2026 |
...
- **UI adicional instalada (listo para usar)**: `@heroui/react` (NextUI), `shadcn/ui` base configurada (`components.json` + `lib/utils.ts`), `recharts`, `embla-carousel-react`, `react-intersection-observer`, `next-themes`
- **Juego**: Canvas 2D vanilla en MuzicMania + `pixi.js` y `phaser` instalados para migración futura
...
|---|---|
| 2026-08-24 | **Visual builders COMPLETADO** (`VISUAL_BUILDERS_SYSTEM.md` v1.2.0): **Plasmic** integrado en las 4 webs como builder local (`@plasmicapp/loader-nextjs@2.0.24`, `/plasmic-host` + `/plasmic/[[...path]]`, envs `PLASMIC_PROJECT_ID`/`PLASMIC_PUBLIC_TOKEN` solo en `.env.local`; sin envs → 404 en Vercel; `/plasmic-host` → notFound). Code components de `@ciszu/ui` registrados en el Studio (Button, RichText, SmartImage) ×4 webs. `PLASMIC_PROJECT_ID=9ibXRSf12whLRbaTQvvxjT` (Ciszu Network Playground) en los 4 `.env.local`. **Onlook POSPUESTO** (demo/self-host pesado; clon en `clones/onlook` gitignored, `bun install` OK, requiere CodeSandbox/OpenRouter/Morph keys + Docker). Verificado: tsc + lint + build OK en las 4 webs; deploy `d00a132a`. |
| 2026-08-23 | **Repo público + fix deploys/sitemaps/CodeQL**: repo pasó a público (reglas AGENTS.md §1/§6.2/§7.1, cero secretos). Deploy **MuzicMania** arreglado (root cause: `.vercelignore` con `**/CHANGELOG*` excluía `changelog.ts`/`changelogIcons.tsx` del upload a Vercel → `Can't resolve '@/data/changelog'`; corregido a `**/CHANGELOG.md` en root+4 webs; además sitemap dinámico `src/app/sitemap.ts` en las 4 webs sin `MetadataRoute` — no disponible en el next instalado — que rompía el build). **Sitemaps GSC**: eliminado el "índice anidado" (el `sitemap.xml` se referenciaba a sí mismo vía `robotsTxtOptions.additionalSitemaps` en `next-sitemap`); quitado `next-sitemap` de las 4 webs; corregido typo `cizukoantony`→`ciszukoantony` en robots/sitemap de ciszukoantony. **CodeQL**: 13 alerts cerrados (path-injection serve-cdn.js, ReDoS stt.js, xss-through-dom preferences.ts, insecure randomness guest.ts ×3, url-sanitization PostHog test, sanitización security.spec); dependabot tmp→0.2.7 (override), extract-zip sin fix (dismissed justificado). **Open source**: enlace "Open Source · Repositorio en GitHub" (https://github.com/Ciszu-Network/CiszuNetwork) en los 4 footers. **Titles auth**: `usePageTitle('LOGIN'/'REGISTER')` creado en ciszu/ciszubot (patrón "Nombre | SECCIÓN" como muzicmania). **Zoom**: barra con botones −/+ en extremos + barra progresiva en los 4 PreferencesPanel. **LanguagesModal**: botón "volver" añadido (arregla modal cortado/que no regresaba). **CDN al cambiar idioma**: ciszubot usa `router.refresh()` en vez de `window.location.reload()` (evita re-montar CloudflareGuard y assets que no cargaban). **DisclaimerStack**: usa `zoomStore` (extraído para romper ciclo de imports) y baja con el header cuando el zoom desplaza `mt-8`. **Isotipo auth antony**: `ANTONY_ISOTYPE` apuntaba a `ciszuko_logo_isotipo_outline_zwhite_ccolor.svg` no subido al CDN → corregido a `_zcolor_ccolor.svg` (200). **Lighthouse CI**: `.lighthouserc.json` con URLs de las 4 webs. Pendiente: terminar tema claro completo en 4 webs, idiomas ES/EN completos, estilo de errores de ciszukoantony. |
...
_Última revisión: 23 ago 2026._ Relacionado: `PROJECTS_SYSTEM.md`, `STATISTICS_SYSTEM.md`,

...
const I = {
  ...
  bot: <svg viewBox="0 0 24 24" className="w-full h-full" fill="none" stroke="currentColor" strokeWidth={2}><rect x="3" y="11" width="18" height="10" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/><circle cx="12" cy="16" r="1"/></svg>,
  ...
  const fetchContent = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const header = format === 'txt' ? `DOCUMENTATION - CISZUBOT PORTAL\nDOC_ID: ${selectedDoc}\nIDENTIFIER: ${identifier}\n------------------------------------------------\n\n` : '';
        const cleanText = text.replace(/^CISZUBOT - DOCUMENTACIÓN OFICIAL[\s\S]*?---\n\n/, '')
                             .replace(/^# CISZUBOT - DOCUMENTACIÓN OFICIAL[\s\S]*?---\n\n/, '');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <aside className="lg:col-span-4 space-y-8 h-full sticky top-32">
          <div className="p-1 rounded-[3rem] bg-gradient-to-br from-white/10 to-transparent shadow-2xl">
            <div className="p-8 bg-[#0a0a0f] border border-white/5 rounded-[2.9rem] flex flex-col h-[750px]">
                ...
                {filteredDocs.map(id => (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-2xl flex items-center gap-4 transition-all group ${
                      selectedDoc === id ? 'bg-neon-blue/10 border border-neon-blue/30 text-neon-blue shadow-[0_0_20px_rgba(0,212,255,0.1)]' : 'hover:bg-white/5 text-gray-500 border border-transparent'
                    }`}
                  >
          ...
          <div className="p-1 rounded-[3rem] bg-gradient-to-br from-neon-purple/20 to-transparent shadow-2xl">
            <div className="p-8 bg-[#0a0a0f] border border-white/10 rounded-[2.9rem] space-y-6">
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="flex justify-between border-b border-white/5 pb-2">
                  ...
                  <span className="text-[9px] font-black text-white font-mono uppercase">CISZUBOT_NODE_{selectedDoc.slice(0,3)}</span>
                </div>
              </div>
        ...
        <main className="lg:col-span-8 space-y-12">
          <motion.div key={selectedDoc} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="p-12 bg-[#0a0a0f] border border-white/10 rounded-[4rem] shadow-[0_30px_100px_rgba(0,0,0,0.5)] relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-end justify-between gap-10">
              <div className="space-y-6 flex-1 min-w-0">
                ...
                <div className="flex items-center gap-6">
                  <div className="w-12 h-12 text-neon-blue flex-shrink-0" style={{ filter: 'drop-shadow(0 0 8px rgba(0,212,255,0.5))' }}>{(I as any)[meta.icon] || I.book}</div>
          ...
          <div className="bg-[#0a0a0f] border border-white/10 rounded-[4rem] shadow-2xl overflow-hidden flex flex-col h-[850px] relative">
            ...
            <div className="flex-1 overflow-auto p-12 md:p-16 bg-black/60 relative group/view">
              {loading ? (
                ...
                <div className="font-mono text-[14px] md:text-base leading-[1.8] text-gray-300">
          ...
          <div className="p-12 bg-[#0a0a0f] border border-white/10 rounded-[4rem] shadow-2xl space-y-12 relative overflow-hidden">

...
const handler = createRecaptchaHandler({
  v2Secret:
    process.env.RECAPTCHA_SECRET_KEY_V2_CISZUBOT ?? process.env.RECAPTCHA_SECRET_KEY_V2 ?? '',
  v3Secret:
    process.env.RECAPTCHA_SECRET_KEY_V3_CISZUBOT ?? process.env.RECAPTCHA_SECRET_KEY_V3 ?? '',
  minScore: 0.5,
  windowMs: 60_000,
  max: 30,
  allowedHostnames: allowedHostnamesFromSiteUrl(
    process.env.NEXT_PUBLIC_SITE_URL ?? 'https://ciszubot.vercel.app',

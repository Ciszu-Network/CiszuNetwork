...
function Show-Modify {
    ...
    Write-Host "${c_gray}Cualquier dato excepto el rango (cargo) se puede modificar. (Enter = vaciar)${c_reset}"
    $empId = Show-PickEmployee "Elige el empleado a MODIFICAR" -ExcludeId $script:identityId
    if (-not $empId) { return }
    $fields = @(
        @{ ic = '✏'; l = 'nombres' },
        @{ ic = '✏'; l = 'apellidos' },

...
Usa el dispatcher `tools/cibersecurity/osint/osint.ps1` (o los wrappers individuales) para
ejecutar herramientas OSINT con los presets oficiales. Salidas:

- **Oficial**: `tools/cibersecurity/osint/output/<herramienta>/`
...
>
> **maltego**: instalado por Ciszuko (v4.12.1 Community). GUI manual vía
> `tools/cibersecurity/maltego/maltego.ps1` (`-Config`/`-Log`). Sin CLI para CI.

## Ejemplos

```powershell
# Maigret full
& "E:\Ciszu Network\tools\cibersecurity\osint\maigret.ps1" -Usernames none_xisty_zzz_999

# Sherlock test rapido
& "E:\Ciszu Network\tools\cibersecurity\osint\sherlock.ps1" -Usernames none_xisty_zzz_999 -Preset quick -Test

# SimpleLogin: validar key + lista de alias
& "E:\Ciszu Network\tools\cibersecurity\osint\simplelogin.ps1" info
& "E:\Ciszu Network\tools\cibersecurity\osint\simplelogin.ps1" aliases

# SpiderFoot por tipo de target
& "E:\Ciszu Network\tools\cibersecurity\osint\spiderfoot.ps1" -Targets foo@example.com -Preset quick

# Maltego (GUI)
& "E:\Ciszu Network\tools\cibersecurity\maltego\maltego.ps1"
```

## Reglas

- Si el usuario no especifica Otra cosa, la salida oficial va a `tools/cibersecurity/osint/output/<herramienta>/`.
- Nunca imprimas la API key de SimpleLogin ni recovery codes; `simplelogin.ps1` los lee del vault.
- Maltego es GUI manual; no hay CLI para CI. Para automatizar, usa SpiderFoot.

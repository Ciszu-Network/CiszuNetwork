...
```powershell
& "E:\Ciszu Network\tools\osint\maigret.ps1" -Usernames <user1>,<user2> [-Preset full|quick] [-Test] [-Out ruta]
```

- **full** (default): `--graph --tags social,tech --csv --json ndjson --html`
- **quick**: solo `--csv`
- Salida oficial: `tools/osint/output/maigret/` · Test: `test/osint/maigret/` (gitignored)

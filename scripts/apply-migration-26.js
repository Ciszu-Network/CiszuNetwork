...
const sql = fs.readFileSync(
  path.resolve(__dirname, '..', 'services', 'supabase', 'migrations', '20260831000026_ads_impressions.sql'),
  'utf8'
).trim();
console.log('Applying migration 26 (ads_impressions, ' + sql.length + ' chars)...');
fetch(API + '/projects/' + REF + '/database/query', {
  ...
  if (r.ok || r.status === 201) {
    console.log('OK: migration 26 applied');

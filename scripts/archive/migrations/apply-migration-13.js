...
const sql = fs.readFileSync(
  path.resolve(__dirname, '..', 'services', 'supabase', 'migrations', '20260801000013_bot_status_heartbeat.sql'),
  'utf8'
).trim();
console.log('Applying migration 13 (' + sql.length + ' chars)...');
fetch(API + '/projects/' + REF + '/database/query', {
  ...
  if (r.ok || r.status === 201) {
    console.log('OK: Migration 13 applied');

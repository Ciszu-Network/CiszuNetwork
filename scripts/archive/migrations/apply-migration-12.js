...
const sql = fs.readFileSync(
  path.resolve(__dirname, '..', 'services', 'supabase', 'migrations', '20260801000012_revoke_execute_security_definer.sql'),
  'utf8'
).trim();
console.log('Applying migration 12 (' + sql.length + ' chars)...');
fetch(API + '/projects/' + REF + '/database/query', {
  ...
  if (r.ok || r.status === 201) {
    console.log('OK: Migration 12 applied');

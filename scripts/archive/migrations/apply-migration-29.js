...
const sql = fs.readFileSync(
  path.resolve(__dirname, '..', 'services', 'supabase', 'migrations', '20260908000029_global_disclaimers_extend.sql'),
  'utf8'
).trim();
console.log('Applying migration 29 (global_disclaimers extend, ' + sql.length + ' chars)...');
fetch(API + '/projects/' + REF + '/database/query', {
  ...
  if (r.ok || r.status === 201) {
    console.log('OK: migration 29 applied');

...
const sql = fs.readFileSync(
  path.resolve(__dirname, '..', 'services', 'supabase', 'migrations', '20260729000010_fix_submit_game_score_invoker.sql'),
  'utf8'
).trim();
console.log('Applying migration 10 (' + sql.length + ' chars)...');
fetch(API + '/projects/' + REF + '/database/query', {
  ...
  if (r.ok || r.status === 201) {
    console.log('OK: Migration 10 applied');

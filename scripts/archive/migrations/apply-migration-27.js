...
const sql = fs.readFileSync(
  path.resolve(__dirname, '..', 'services', 'supabase', 'migrations', '20260902000027_global_disclaimers.sql'),
  'utf8'
).trim();
console.log('Applying migration 27 (global_disclaimers, ' + sql.length + ' chars)...');
fetch(API + '/projects/' + REF + '/database/query', {
  ...
  if (r.ok || r.status === 201) {
    console.log('OK: migration 27 applied');

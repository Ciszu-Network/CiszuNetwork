...
const sql = fs.readFileSync(
  path.resolve(__dirname, '..', 'services', 'supabase', 'migrations', '20260904000028_rls_hardening.sql'),
  'utf8'
).trim();
console.log('Applying migration 28 (rls_hardening, ' + sql.length + ' chars)...');
fetch(API + '/projects/' + REF + '/database/query', {
  ...
  if (r.ok || r.status === 201) {
    console.log('OK: migration 28 applied');

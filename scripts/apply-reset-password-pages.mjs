/**
 * Genera la página `/reset-password` de las 4 webs a partir de UNA plantilla.
 *
 * POR QUÉ UN GENERADOR: las 4 páginas habían divergido tanto que una de ellas
 * ni siquiera compilaba (`toast` sin definir en ciszubot) y otra validaba la
 * contraseña en el login en vez de en una pantalla propia. Repartir el mismo
 * comportamiento en 4 sitios a mano es cómo se llegó a ese estado; aquí la
 * plantilla es la única fuente de verdad y el script solo cambia la marca.
 *
 * Uso: node scripts/apply-reset-password-pages.mjs
 */
...
// fileURLToPath decodifica los escapes (el repo vive en "Ciszu Network", con espacio).
const ROOT = fileURLToPath(new URL('..', import.meta.url));

const CISZU_ISOTYPE =
  'projects/ciszu/content/logos/images/outline/isotype/color/ciszu_logo_isotipo_outline_zwhite_ccolor.svg';

/** Marca de cada web: solo cambia esto. */
const SITES = {
  ciszu: {
    name: 'Ciszu Network',
    tagline: 'Recupera el acceso a tu CISZU ID',
    isotipo: `'${CISZU_ISOTYPE}'`,
    secondaryIsotipo: null,
    cdnImport: null,
    accent: 'from-neon-blue to-neon-purple',
  },
  ciszubot: {
    name: 'CiszuBot',
    tagline: 'Recupera el acceso a tu cuenta de CiszuBot',
    isotipo: `'${CISZU_ISOTYPE}'`,
    secondaryIsotipo: null,
    cdnImport: null,
    accent: 'from-[#5865F2] to-neon-blue',
  },
  ciszukoantony: {
    name: 'Ciszuko Antony',
    tagline: 'Recupera el acceso a tu cuenta de Ciszuko Antony',
    isotipo: `assetResolver.resolve('${CISZU_ISOTYPE}')`,
    secondaryIsotipo: `assetResolver.resolve('projects/ciszukoantony/content/logos/images/outline/isotype/color/ciszuko_logo_isotipo_outline_zwhite_ccolor.ai.svg')`,
    cdnImport: "import { assetResolver } from '@ciszunetwork/cdn';",
    accent: 'from-neon-pink via-[#6600ff] to-neon-blue',
  },
  muzicmania: {
    name: 'MuzicMania',
    tagline: 'Recupera el acceso a tu cuenta de MuzicMania',
    isotipo: `resolveAssetPath('${CISZU_ISOTYPE}')`,
    secondaryIsotipo: null,
    cdnImport: "import { resolveAssetPath } from '@ciszunetwork/cdn';",
...
const __ISOTYPE_CONST__ = __ISOTYPE_EXPR__;
__SECOND_ISOTYPE_BLOCK__
  ...
  return (
    <div className="bg-bg min-h-[calc(100vh-60px)] relative overflow-hidden pb-24">
      ...
      <div className="pt-14 mb-8 px-4 flex items-center justify-center gap-3">
        <SmartImage src={__ISOTYPE_CONST__} alt="Ciszu ID" width={40} height={40} className="w-9 h-9" />
__SECOND_LOGO_JSX__
      </div>
      ...
      <div className="max-w-md mx-auto px-4 relative">
        <div className="text-center mb-6 space-y-1">
          ...
          <p className="text-gray-400 text-[11px] font-bold">__SITE_TAGLINE__</p>
        </div>

        <div className="relative">
          <div className="absolute -inset-1 bg-gradient-to-r __ACCENT__ rounded-[2rem] blur opacity-20" />
...
for (const [site, cfg] of Object.entries(SITES)) {
  const target = join(ROOT, 'projects', site, 'website', 'src', 'app', 'reset-password', 'page.tsx');
  if (!existsSync(join(ROOT, 'projects', site, 'website'))) {
    console.log(`skip ${site}: no website`);
    continue;
  }

  const isotipoConst = site === 'ciszukoantony' ? 'CISZU_ISOTYPE' : 'CISZU_ISOTYPE';
  const secondBlock = cfg.secondaryIsotipo
    ? `const ANTONY_ISOTYPE = ${cfg.secondaryIsotipo};\n`
    : '';
  const secondLogoJsx = cfg.secondaryIsotipo
    ? `        <SmartImage src={ANTONY_ISOTYPE} alt="${cfg.name}" width={40} height={40} className="w-9 h-9" />`
    : '';

  // replaceAll a propósito: varios marcadores aparecen más de una vez (el
  // nombre de la web sale en el encabezado y en el pie) y `replace` con un
  // patrón de texto solo sustituye la PRIMERA aparición.
  const content = PAGE.replaceAll('__CDN_IMPORT__', cfg.cdnImport ?? '')
    .replaceAll('__ISOTYPE_CONST__', isotipoConst)
    .replaceAll('__ISOTYPE_EXPR__', cfg.isotipo)
    .replaceAll('__SECOND_ISOTYPE_BLOCK__', secondBlock.trimEnd())
    .replaceAll('__SECOND_LOGO_JSX__', secondLogoJsx)
    .replaceAll('__SITE_NAME__', cfg.name)
    .replaceAll('__SITE_TAGLINE__', cfg.tagline)
    .replaceAll('__ACCENT__', cfg.accent);

  const leftovers = content.match(/__[A-Z_]+__/g);
  if (leftovers) {
    throw new Error(`Marcadores sin sustituir en ${site}: ${[...new Set(leftovers)].join(', ')}`);
  }
...
console.log(`\n${written} páginas de recuperación generadas.`);

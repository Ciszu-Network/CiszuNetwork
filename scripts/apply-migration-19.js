...
const sql = fs.readFileSync(
  path.resolve(__dirname, '..', 'services', 'supabase', 'migrations', '20260819000019_puck_pages_app.sql'),
  'utf8'
).trim();
console.log('Applying migration 19 (' + sql.length + ' chars)...');
fetch(API + '/projects/' + REF + '/database/query', {
  ...
  if (r.ok || r.status === 201) {
    console.log('OK: migration 19 applied');

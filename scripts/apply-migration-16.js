...
const sql = fs.readFileSync(
  path.resolve(__dirname, '..', 'services', 'supabase', 'migrations', '20260810000016_security_hardening.sql'),
  'utf8'
).trim();
console.log('Applying migration 16 (' + sql.length + ' chars)...');
fetch(API + '/projects/' + REF + '/database/query', {
  ...
  if (r.ok || r.status === 201) {
    console.log('OK: Migration 16 applied');

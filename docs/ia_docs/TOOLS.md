...
### 6.1 Bruno — `api-tests/` (ya configurado en el repo)
...
- **Ejecutar**: `pnpm api:test` (raíz) → `bru run api-tests --env prod`
...
> **CLI**: `pnpm add -g @usebruno/cli` (instalado 4.0.0). Requiere `PNPM_HOME` en PATH (`pnpm setup`; `C:\Users\fplay\AppData\Local\pnpm\bin`). El wrapper `scripts/run-bru.js` ejecuta `bru` con cwd `api-tests/` y crea el dir de `--reporter-json`.
...
- ✅ **`scripts/backup-db.js` reparado**: usaba el endpoint muerto `/database/connection` (404). Ahora usa `/config/database/pooler` + `SUPABASE_DB_PASSWORD` de `.env`, con fix CRLF en el parser. `pg_dump` encontrado en `E:\DaVinci\PGTools\pg_dump.exe` (13.4 — ⚠️ **incompatible con server 17.6**, "server version mismatch"; requiere pg_dump ≥17).
...
- ⏳ Opcional: configurar MCP server de dbvr (§6.3) para acceso IA directo a la BD
- ⏳ Opcional: instalar PostgreSQL 17 (o Docker con `postgres:17`) para `pg_dump` compatible — el backup real sigue pendiente de eso

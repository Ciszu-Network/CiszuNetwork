...
1. **`.env`**: mover `apps/ciszubot/discord-bot/.env` al VPS (nunca a git).
...
2. Conectar por SSH. Instalar:
   ...
   ```
3. Subir el repo (o solo `apps/ciszubot/discord-bot/` + `docker-compose.yml` + `.env`):
   ```bash
   git clone https://github.com/Ciszu-Network/CiszuNetwork.git
   cd CiszuNetwork
   cp apps/ciszubot/discord-bot/.env.example apps/ciszubot/discord-bot/.env
   nano apps/ciszubot/discord-bot/.env   # pegar token real
   ```
4. Arrancar:
   ```bash

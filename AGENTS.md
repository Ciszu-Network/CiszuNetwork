...
**Nunca Redis: caché multi-tienda en `packages/utils` (nuevo, 0 deps)**: memoria LRU (siempre) → **Vercel KV REST** (activo — store `upstash-kv-ciszunetwork` conectado a muzicmania y ciszubot, verificado 10 ago 2026) → **Postgres `ciszu.cache`** (schema `ciszu`, migración 15 aplicada). Doc: `projects/ciszu/docs/documentation/CACHING_SISTEMA.md`.

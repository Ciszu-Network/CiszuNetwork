...
- **`NEXT_PUBLIC_` viaja al bundle del navegador**: cualquier secret con ese prefijo es público (lección tarea 2, 10 ago). Secrets solo como `process.env.X` sin prefijo en server-only. Los fallbacks hardcodeados de secrets en código quedan en git para siempre (decisión pendiente: rotar el secret de Turnstile de muzicmania cuando el repo se haga público).
...
- ✅ **Vercel token `vcp_` ROTADO** (10 ago 2026) — nuevo token guardado como `VERCEL_TOKEN` en `.env.local` (raíz) **y** GH secret `VERCEL_TOKEN` del repo actualizado con el mismo valor (verificado `gh secret list`). **NO volver a rotar este token** (ya está documentado y activo). Restan en la lista de rotación: service*role, JWT secret, password dashboard, anon (Supabase) — regenerar en Dashboard y actualizar `.env` con `scripts/update-env-keys.js` — Suno AI key, Cloudflare R2, Discord. Lista: `scripts/tokens_a_rotar.md` (sin valores) + valores en `Temp\opencode\tokens_a_rotar.md`

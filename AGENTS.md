...
- **`opencode-run` es el lanzador por defecto de sesiones** (5 ago 2026, `scripts/opencode-run.cmd` + copia en PATH): mata el listener de 4096 si escucha (recarga de config) → **delega en `opencode-ciszu-pc`** (ensure server + attach al puerto 4096), reutilizando su lógica de attach para no dejar ventanas huérfanas. Comportamiento según origen: **consola local del PC** → se auto-eleva a administrador (UAC, para que opencode corra elevado); **sesión remota SSH/Termius** → NO eleva (detecta `SESSIONNAME` no `Console`) y hace el mismo flujo, así el móvil la usa igual que `opencode-ciszu-cel`. El server `opencode serve :4096` corre sin elevar (tarea `opencode-server-ciszu` InteractiveToken) → matarlo no requiere admin.
- **Permisos de opencode sin prompts** (5 ago 2026): config global `~/.config/opencode/opencode.json` con `"permission": { "*": "allow", "external_directory": "allow", ... }` → nunca pregunta "allow" para acceder o editar fuera del workspace (rutas `C:\Users\fplay\...`, etc.). Se mantienen `doom_loop`/`question` en `ask` y lectura de `*.env` denegada (secrets del vault). Aplica al reiniciar con `opencode-run`.

...
- **Verificación**: builds 4 webs OK (8 ago 2026, 3er fix: CSS autocontenido + tests actualizados a estilo inline), manifest+theme-color+sw.js+iconos 200 en `next start` local. Tests 91/91 (incl. `packages/ui/tests/InstallPdwaButton.test.tsx` — navegador, dismiss, prompt nativo, disclaimers, app nativa y posicionamiento inline `fixed`/`blur`), lint OK (ciszubot web no tiene script lint propio)

...
- **CI** (`.github/workflows/ci.yml`) — lint, unit tests (Vitest 157), **seguridad concurrente**: semgrep (SAST), `pnpm audit --prod` (SCA), **gitleaks** (secret scanning historial completo — GitHub Secret Scanning nativo NO está en repos privados Free), **security-e2e** (Playwright DAST interactivo contra las 4 webs en prod: cabeceras, no-reflejo XSS/SQLi, paths de escáneres; espera hasta 180s a los deploys en CI). Spec: `test/website/e2e/security.spec.ts`

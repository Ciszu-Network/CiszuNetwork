...
- **`opencode-run` es el lanzador por defecto de sesiones** (5 ago 2026, `scripts/opencode-run.cmd` + copia en PATH): mata el listener de 4096 si escucha → ensure → attach. Comportamiento según origen: **consola local del PC** → se auto-eleva a administrador (UAC, para que opencode corra elevado); **sesión remota SSH/Termius** → NO eleva (detecta `SESSIONNAME` no `Console`) y hace el mismo ensure+attach, así el móvil la usa igual que `opencode-ciszu-cel`. El server `opencode serve :4096` corre sin elevar (tarea `opencode-server-ciszu` InteractiveToken) → matarlo no requiere admin.

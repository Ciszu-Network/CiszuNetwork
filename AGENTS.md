...
- ⚠️ **Repo PÚBLICO (desde 23 ago 2026)**: todo lo que se commitea es visible para el mundo.
  NUNCA commitear secretos, tokens, API keys, `.env*`, URLs con credenciales ni datos
  personales. Los secretos viven solo en el vault cifrado local + Bitwarden; en el repo
  solo pueden existir referencias por nombre (ver §6.5 y §7.1).
...
| Editor visual UI     | `VISUAL_BUILDERS_SYSTEM` · `UI_COMPONENTS_SYSTEM` · `GLOBAL_COMPONENTS_SYSTEM` · `STYLES_SYSTEM` · `COLOR_SYSTEM`        |
...
- **Sistemas**: `ARCHITECTURE.md` · `FULL_STACK_SYSTEM` (stack) · `DB_SYSTEM` · `ORM_SYSTEM` (Drizzle) ·
  ...
  `GOOGLE_SYSTEM` (negocio y reseñas de Google) · `GLOBAL_ADVISOR_SYSTEM` (mensajes globales del admin a las webs) ·
  ...
  `STAFF_SYSTEM` (gestión de empleados: archives/staff + STAFFCON) · `EMPLOYEES_SYSTEM` (modelo organizacional: cargos/rangos/permisos/horarios) · `CUSTOMERS_SYSTEM` (gestión de clientes: archives/customers + CUSTOMERSCON) · `AD_SYSTEM` (anuncios: intrusivos/particulares/recompensa/opcionales + GA4) · `MONETIZATION_PROTOCOLS` (monetización: donaciones, anuncios, compras/suscripciones futuras) · `CHANGELOG_SYSTEM` (registro de cambios: índice/detalle, estado, roadmap, likes)
- **Arquitectura por capas**: `FRONTEND_SYSTEM` · `BACKEND_SYSTEM` · `PACKAGES_SYSTEM` ·
  `UI_COMPONENTS_SYSTEM` (Storybook/Chromatic/Figma/Tailwind/React) · `GLOBAL_COMPONENTS_SYSTEM`
  (sistemas globales: disclaimers, toasts, FABs, theme/lang, navbar/footer, zoom/island) · `FRAMEWORKS_SYSTEM` · `STYLES_SYSTEM` · `COLOR_SYSTEM`
- **Planes**: `COMPANY_REGISTRATION_PLAN` (+ `RIF_PERSON_PLAN` · `TRADEMARK_PLAN` ·
  ...
  `SEO_PLAN` (pipeline automatizado: Screaming Frog CLI + scripts `seo-*`, carpetas `website/seo/`)
...
├── archives/            # Backups y archivos grandes (gitignored parcialmente); staff/ = empleados
...
clientes reales: estructura `archives/customers/` (global → cliente, 5 formatos por nivel +
...
Package manager: **pnpm v10.8.1**, Node >=24. Todas las webs: Next.js 15 (App Router) +
...
- ⚠️ **Repo PÚBLICO**: antes de cada commit, revisar `git status`/`git diff` y confirmar que
  ...
  Si un secreto se filtra por error en un push: **rotarlo ya** y anotarlo en el registro de
  incidentes (no basta con borrarlo del history).
...
### 6.6 El vault de secrets (`services/supabase/.env`) — edición con permiso

- **`services/supabase/.env`** es el vault local de credenciales (descifrado para el flujo
  diario; la copia cifrada es `.env.age`). Ciszuko puede escribirlo directamente; el agente
  **por defecto solo lo lee** (usar variables al vuelo, `vault.ps1 crypt|verify|backup`).
- **Autorización (01 sep 2026)**: Ciszuko autorizó al agente a **editar `services/supabase/.env`**
  cuando una tarea lo requiera (p. ej. corregir una línea mal formateada que rompe el CDN,
  rotar/actualizar tokens, añadir entradas nuevas). Al editar: **no borrar ni reordenar
  entradas existentes**; corregir solo lo necesario; mantener el formato `<KEY>=<valor>`
  (sin comentarios pegados en la misma línea de una variable); y tras cualquier cambio
  ejecutar `vault.ps1 crypt` para re-sincronizar `.env.age` y verificar con `vault.ps1 verify`.
- Excepción histórica mantenida: al añadir secretos nuevos entregados por SECRET_TEMP, se
  añaden **al final** del `.env` (mismo nombre de variable) + `vault crypt` + Bitwarden.
  Si una modificación es dudosa, preguntar antes.
...
### 6.8 Skills del agente (estado actual)

**El agente tiene 34 skills instaladas** en `E:\Ciszu Network\.opencode\skills/` (local,
gitignored): 33 instaladas el 31 ago 2026 desde 4 fuentes clonadas en `clones/`
(superpowers, anthropic-skills, jjmartres-opencode, opencode-agents) + la pre-existente
`puck`. Catálogo completo, links de descarga y protocolo de instalación:
`MODELS_SKILLS_SYSTEM.md`. **MCP clients: ausentes** (decisión documentada en ese doc).

- **Cómo se activan**: el loader de opencode escanea `**/SKILL.md` al arranque. Cualquier
  skill nueva/borrada requiere **reiniciar opencode**. Se invocan bajo demanda vía la tool
  `skill` (el modelo decide por `name` + `description`).
- **Reglas**: nunca instalar una skill con secretos (es markdown legible); verificar que
  `name` coincida con el folder y tenga `description`; clonar repos en `clones/` (gitignored,
  shallow); no instalar libs de runtime sin confirmar (`AGENTS.md` §7.1).

### 6.9 Multiagencia y coexistencia de IAs

**El ecosistema ya no es uniagente ni unimodelo.** Actualmente pueden operar simultáneamente
varias interfaces de IA sobre el mismo monorepo: **Kilo CLI** (capa sobre OpenCode + StepFun
Step-3.7 Flash), **OpenCode Go** (DeepSeek V4 Flash), **Freebuff** (Codebuff/DeepSeek V4 Flash),
**OpenCode local**, y posibles agentes adicionales como assistants de IDE, bots o scripts
automatizados. Esta multiplicidad exige reglas explícitas de coordinación.

#### 6.9.1 Reglas de coexistencia

1. **Unidad de autoría**: Ciszuko Antony es el único creador legítimo. Las IAs son herramientas
   de ejecución; ninguna IA firma commits, diseña arquitectura ni toma decisiones de producto.
2. **AGENTS.md como fuente común**: todos los agentes que toquen el repo deben respetar este
   archivo. Si un agente trae su propio archivo de reglas (`CLAUDE.md`, `.agents/AGENTS.md`,
   etc.), este `AGENTS.md` prevalece sobre el resto.
3. **Locking por archivo/carpeta**: antes de modificar un archivo, verificar quién lo está
   tocando. Si hay otro agente en la misma sesión/tarea, coordinar por diff/resumen y no
   sobrescribir trabajo en caliente. En caso de duda, preguntar a Ciszuko Antony.
4. **Commits atómicos y documentados**: cada commit debe indicar agente/proveedor cuando sea
   relevante, p. ej. `feat(...): ... [agent:Kilo/StepFun]`. Esto evita bucles de recreación
   y permite auditar quién hizo qué.
5. **No limpiar sin confirmar**: no borrar archivos, carpetas `.opencode/temp/*`, scripts
   residuales, ramas ni sesiones de otros agentes sin confirmación explícita de Ciszuko Antony.
   Muchos archivos viejos pueden ser knowledge files, skills, datos de sesión o migraciones
   diferidas de otras IAs.
6. **Temporales por agente**: cada IA debe usar su propio subdirectorio dentro de
   `E:\Ciszu Network\.opencode\temp/<agent>/` para evitar colisiones. `tmp/` global es
   compartido y debe tratarse como zona común.
7. **Sesiones y handovers**: cuando un agente ceda el turno a otro, dejar un resumen en
   `PROJECT_STATE.md` y/o el chat con: archivos modificados, comandos ejecutados, estado de
   build/lint, y próximos pasos sugeridos. No dar por terminada una tarea solo porque el
   código “parece” listo; verificar build real.

#### 6.9.2 Riesgos multiagente y mitigaciones

| Riesgo | Mitigación |
|---|---|
| Dos agentes editan el mismo archivo | Verificar `git status`/`git diff` antes de editar; si hay conflicto, coordinar |
| Recreación de archivos borrados por otro agente | Preguntar antes de borrar; usar `git log` para ver quién eliminó qué |
| Duplicación de tareas | Leer `TODO.md` y `PROJECT_STATE.md` antes de iniciar una tarea |
| Inyección de prompts o skills maliciosas | Solo instalar skills desde fuentes confiables; inspeccionar `SKILL.md` |
| Consumo excesivo de contexto/cuota | Respetar límites de sesión (§9) y cerrar/archivar tareas terminadas |
| Secretos compartidos entre agentes | Nunca compartir `.env` ni vault; cada agente accede por referencia, no por valor |
| Workflows de CI/CD solapados | Los deploys pasan por GitHub Actions; no ejecutar deploys locales en paralelo |
| Locking no declarado | Registrar en `PROJECT_STATE.md` o chat el archivo/carpeta bloqueada y el motivo |
| Resolución de conflictos | El agente que llegue segundo debe pedir el diff del primero y reaplicar sus cambios sobre esa base |

##### Reglas adicionales de locking

1. Antes de modificar un archivo, verificar si está siendo editado por otro agente mediante
   `git status`, `git diff` o consulta en el chat.
2. Si un agente va a modificar un archivo por más de 10 minutos, debe registrar el lock en
   `PROJECT_STATE.md` o en el chat compartido.
3. Los directorios `.opencode/temp/<agent>/` y `tmp/<agent>/` son de uso exclusivo por agente.
   No tocar el directorio de otro agente sin confirmación.
4. Si se detecta un conflicto de edición, detener la escritura y solicitar una base común
   mediante diff/patch antes de continuar.
...
- NUNCA imprimir `.env`/tokens en logs o resúmenes; referirse genéricamente. Si un secreto se
  filtra: rotarlo y registrarlo en el registro de incidentes de seguridad.
- ⚠️ **Repo PÚBLICO — cero secretos en el repo**: los valores de keys/tokens solo viven en el
  vault cifrado local (`services/supabase/.env` + `.env.age`) y Bitwarden; los `.env.local`
  de desarrollo son gitignored. En el código y docs solo referencias por nombre
  (`process.env.X`). Nunca incluir valores reales en commits, issues, PRs ni docs. Verificar
  `git diff` antes de cada commit (gitleaks/secretlint bloquean `--staged`).
...
_Última revisión: 23 ago 2026._ Fuente de verdad: `projects/ciszu/docs/documentation/`.

...
- **Errores = Sentry** (Developer free 5k/mes, sin tarjeta). `@sentry/nextjs@10.69` en las 4 webs (configs en `src/`: client/server/edge + `instrumentation.ts` con `register()` + `onRequestError`/`captureRequestError` + `global-error.tsx`; `next.config.ts` con `withSentryConfig({org:'ciszu-network', project:<app>, silent:true, sourcemaps:{disable:!process.env.SENTRY_AUTH_TOKEN, filesToDeleteAfterUpload:['.next/static/**/*.map']}})`; muzicmania usa global-error HTML propio — su shim legacy rompe `next/error`) y `@sentry/node@10.69` en el bot (`src/services/sentry.ts`, solo con `SENTRY_DSN`, traces 0.1). **✅ ACTIVADO (11 ago 2026)**: org `ciszu-network` + 5 proyectos (ciszunetwork, ciszukoantony, muzicmania, ciszubot, ciszubot-bot), DSNs en `.env.local` ×4 + `.env` bot + Vercel ×4 (production+preview+dev) + `SENTRY_AUTH_TOKEN` (solo production), source maps subiéndose en builds. **Todas las features ON por decisión del usuario**: tracing 1.0 (server/edge/client), replays (client 0.1 / onerror 1.0) + `feedbackIntegration` (widget "Reportar un problema"), bot traces 0.1. Tokens en vault cifrado (`SENTRY_ORG_TOKEN` `sntrys_…`, `SENTRY_PERSONAL_TOKEN` `sntryu_…`). PostHog queda analítica pura. `pnpm-workspace.yaml` aprobó `@sentry/cli`. Builds 5/5 OK. Sin DSNs el SDK es no-op.

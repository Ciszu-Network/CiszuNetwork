...
- **`ciszu-ai` es el lanzador oficial de sesiones** (7 ago 2026, `tools/ciszu-ai/ciszu-ai.cmd`; en PATH hay stub `AppData\Roaming\npm\ciszu-ai.cmd`): por defecto mata el listener de 4096 si escucha (recarga de config) → **`ensure-server.ps1`** (levanta el server si no responde) → `opencode attach http://127.0.0.1:4096`. Subcomandos: `ciszu-ai server` (solo garantizar/levantar el server sin attach) y `ciszu-ai stop`. **Siempre abre la sesión local DENTRO de Windows Terminal** (fuente correcta, UTF-8 y `Ctrl+Enter` como salto de línea): si no eres admin relanza `wt.exe -Verb RunAs` (UAC); si ya eres admin abre `wt.exe` directo; solo si ya estás dentro de WT (variable `WT_SESSION`) ejecuta el reload in-place. **Sesión remota SSH/Termius** → NO toca WT (usa su propio terminal) y NO eleva (detecta `SESSIONNAME` no `Console`). `chcp 65001` al inicio. El server `opencode serve :4096` corre sin elevar (tarea `opencode-server-ciszu` InteractiveToken) → matarlo no requiere admin. `opencode-run` / `opencode-ciszu-pc` / `opencode-ciszu-cel` son **alias de compatibilidad** que delegan en `ciszu-ai` (el viejo lanzador standalone se eliminó; la lógica vive solo en `tools/ciszu-ai/`).
